...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Systemcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Systemcoin Signed Message:\n";
...
{
    // Systemcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Systemcoin: 1 day
static const int64 nTargetSpacing = 60; // Systemcoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Systemcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Systemcoin - DarkGravity v2, written by Evan Duffield - evan@Systemcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Systemcoin - DarkGravity v3, written by Evan Duffield - evan@Systemcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // Systemcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Systemcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x0000092c00976daebaceb04f34a31e487ba95017bc0ca0e721f55721a21b6079");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Systemcoin is developed for freedom!!! <3 ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1488027600; // Saturday, 25. February 2017 13:00:00 GMT
...
//End
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x104411745ee9e06a2d20ad407f887f28680323aceebb5596fa8a87afb6a08d76"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SystemcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SystemcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SystemcoinMiner(CWallet *pwallet)
{
    printf("SystemcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Systemcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SystemcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SystemcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SystemcoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - PEOPLESCURRENCY **********
    Copyright 2014, Bitblock Developers 
        eduffield - evan@Systemcoin.io
        InternetApe - kyle@Systemcoin.io
*/
